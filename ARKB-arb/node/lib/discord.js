...
async function init() {
  ...
  client.once('ready', async () => {
    ...
    try {
      ...
      let channel = channels.find(c => c.name === 'arkb-arb-alerts' && c.isTextBased());
      if (!channel) {
...
async function sendTradeAlert(trade) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: 'ARKB Price',   value: `$${trade.arkbPrice.toFixed(4)}`,    inline: true },
      ...
      { name: 'Unit Size',    value: `5,000 shares`,                     inline: true },
    )
